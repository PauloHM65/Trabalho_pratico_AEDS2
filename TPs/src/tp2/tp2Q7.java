...
public class tp2Q7 {
    ...
    private static int comparar(Restaurante r1, Restaurante r2) {
        comparacoes++;
        int cmp = r1.getCidade().compareTo(r2.getCidade());
        if (cmp == 0) {
            comparacoes++;
    ...
    private static void merge(Restaurante[] arr, int esq, int meio, int dir) {
        int n1 = meio - esq + 1;
        int n2 = dir - meio;
        ...
        for (int i = 0; i < n1; ++i) { L[i] = arr[esq + i]; movimentacoes++; }
        for (int j = 0; j < n2; ++j) { R[j] = arr[meio + 1 + j]; movimentacoes++; }
    ...
    public static void main(String[] args) {
        ...
        mergeSort(subColecao, 0, n - 1);
        long fimTempo = System.currentTimeMillis();
        
        for (int i = 0; i < n; i++) System.out.println(subColecao[i].formatar());
        
        try (FileWriter log = new FileWriter("matricula_mergesort.txt")) {
