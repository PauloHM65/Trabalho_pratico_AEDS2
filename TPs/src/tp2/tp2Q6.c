...
int main() {
    ...
    for (int i = 0; i < n_sub - 1; i++) {
        ...
        for (int j = i + 1; j < n_sub; j++) {
            if (strcmp(sub_colecao[j]->nome, sub_colecao[min_idx]->nome) < 0) {
                min_idx = j;
            }
        }
        if (min_idx != i) {
            Restaurante* temp = sub_colecao[i];
            sub_colecao[i] = sub_colecao[min_idx];
            sub_colecao[min_idx] = temp;
        }
    }
    ...
    while (scanf(" %[^\r\n]", input) == 1) {
        ...
        while (esq <= dir) {
            ...
            int cmp = strcmp(sub_colecao[meio]->nome, input);
    ...
    if (log != NULL) {
        fprintf(log, "1234567\t%d\t%.2f\n", comparacoes_pesquisa, tempo_execucao);
        fclose(log);
    }
