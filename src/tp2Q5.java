...
public class tp2Q5 {
    public static void main(String[] args) {
        ...
        int comparacoes = 0;
        long inicioTempo = System.nanoTime(); // nanoTime garante precisão para buscas curtas
        ...
        while (sc.hasNextLine()) {
            ...
            for (int i = 0; i < n; i++) {
                comparacoes++;
                if (subColecao[i].getNome().equals(nomeBuscado)) {
                    encontrado = true;
                    break;
                }
            }
        ...
        double tempoExecucaoMs = (fimTempo - inicioTempo) / 1_000_000.0;

        try (FileWriter log = new FileWriter("matricula_sequencial.txt")) {
            log.write("827112\t" + comparacoes + "\t" + tempoExecucaoMs + "\n");
        } catch (IOException e) {
            e.printStackTrace();
        }
