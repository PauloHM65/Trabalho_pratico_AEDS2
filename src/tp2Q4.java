...
public class tp2Q4 {
    public static void main(String[] args) {
        ...
        for (int i = 1; i < n; i++) {
            Restaurante tmp = subColecao[i];
            movimentacoes++; 

            int j = i - 1;
            while (j >= 0) {
                comparacoes++;
                if (subColecao[j].getCidade().compareTo(tmp.getCidade()) > 0) {
                    subColecao[j + 1] = subColecao[j];
                    movimentacoes++;
                    j--;
                } else {
                    break;
        ...
        for (int i = 0; i < n; i++) {
            System.out.println(subColecao[i].formatar());
        }

        try (FileWriter log = new FileWriter("matricula_insercao.txt")) {
            log.write("827112\t" + comparacoes + "\t" + movimentacoes + "\t" + tempoExecucao + "\n");
        } catch (IOException e) {
            e.printStackTrace();
        }
