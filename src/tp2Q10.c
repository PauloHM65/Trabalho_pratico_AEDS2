...
int obter_maior_capacidade(Restaurante* arr[], int n) {
...
void counting_sort(Restaurante* arr[], int n) {
    ...
    int max = obter_maior_capacidade(arr, n);
    ...
    for (int i = n - 1; i >= 0; i--) {
        output[count[arr[i]->capacidade] - 1] = arr[i];
...
int main() {
    ...
    clock_t start = clock();
    counting_sort(sub_colecao, n_sub);
    clock_t end = clock();
